...
class BarcodeGenerator:
    ...
    def generate_barcode_base64(self, doc_id: str) -> str:
        ...
        try:
            ...
            # Generate barcode with custom options - wider and shorter
            barcode_instance = code128(doc_id, writer=ImageWriter())
            
            # Optimized writer options for better appearance
            writer_options = {
                'module_width': 0.4,       # Wider bars for better scanning
                'module_height': 8.0,      # Shorter height (was 12.0)
                'quiet_zone': 4.0,         # Smaller margins
                'font_size': 11,           # Larger, clearer text
                'text_distance': 4.5,      # More space between bars and text
                'background': 'white',
                'foreground': 'black',
    ...
    def generate_barcode_file(self, doc_id: str) -> Optional[Path]:
        ...
        try:
            ...
            # Same optimized options as base64 version
            writer_options = {
                'module_width': 0.4,
                'module_height': 8.0,
                'quiet_zone': 4.0,
