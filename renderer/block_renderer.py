...
class NotionBlockRenderer:
    ...
    def render_rich_text(self, rich_text: List[Dict]) -> str:
        """Render Notion rich text with formatting"""
        if not rich_text:
            return ''
        ...
        for text_obj in rich_text:
            ...
            if annotations.get('code'):
                content = f'<code>{self._escape_html(content)}</code>'
    ...
    def _render_code(self, data: Dict) -> str:
        """Render code block with proper escaping - Notion style"""
        ...
        # Notion-style code block with light background
